...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        // Floor the capture width well above this slot's own rendered size:
        // the same uploaded photo is often reused at a much larger size
        // elsewhere on the site (e.g. a small admin thumbnail whose image
        // also becomes a full-width hero/gallery photo on public pages).
        // Encoding at only the visible slot's width would bake in a
