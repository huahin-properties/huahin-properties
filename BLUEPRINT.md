...
- ปุ่ม "+ เพิ่มทรัพย์ใหม่" เปิดฟอร์มกรอก: ประเภท/โซน/ชื่อทำเล/สถานะ/ราคา/ห้องนอน-น้ำ/พื้นที่/ที่ดิน/รายละเอียด
  **+ อัปโหลดรูปสูงสุด 10 รูป** (native file input หลายไฟล์ ต่อคิว, ลบรูปเดิม/รูปใหม่ได้ก่อนบันทึก) —
  รูปเก็บผ่าน Firebase Storage เหมือนระบบ AI Quick Add เดิม (`fileToDataUrl` + `savePhoto`)
- **บันทึกสำเร็จแล้วเข้าหน้าถัดไปทันที** (ไม่ใช่กลับไปหน้ารายการเลย): แสดงข้อความยืนยัน + ช่อง Assisted Sale
  (หมวด 2.1) + **ร่างข้อความโพสต์ขายพร้อมคัดลอก** — ระบบประกอบข้อความจากข้อมูลทรัพย์ที่เพิ่งกรอก (ประเภท/ราคา/
  ห้อง/พื้นที่/รายละเอียด/รหัสทรัพย์) ต่อท้ายด้วย **บล็อกช่องทางติดต่อ + แฮชแท็กมาตรฐานของเว็บ** (ดึงจาก
  Site Content → Facebook post footer เดิมที่มีอยู่แล้ว ซึ่งมีทั้งเบอร์ติดต่อ/LINE/เว็บไซต์/แฮชแท็กรวม
  `#HuaHinProperty` ฯลฯ) มีปุ่ม "คัดลอกข้อความ" กดแล้วก็อปไปวางที่ Facebook/LINE ได้เลย
...
**ยังไม่ได้ทำ**: ลบทรัพย์ทั้งใบ (ตอนนี้แก้ไขได้แต่ลบไม่ได้), จำกัดจำนวนโพสต์ตามแพ็กเกจ (ยังโพสต์ได้ไม่จำกัดจำนวน
ไม่ว่าจะแพ็กเกจไหน — ต้องผูกกับ `tier`/quota ทีหลัง), แจ้งเตือนอีเมลเชิงรุกสำหรับ nudge (ตอนนี้แค่ป้ายในการ์ด)
