...
4. ~~**Stripe subscription + webhook + payments ledger**~~ ✅ **เสร็จแล้ว (โค้ดพร้อม รอ deploy + ตั้งราคา)** —
   ...
   - **สิ่งที่ต้องทำก่อนใช้งานได้จริง (ยังไม่ได้ทำ — ต้องทำเอง)**:
     1. สร้าง Product + Price ใน Stripe Dashboard สำหรับแพ็กเกจ Pro และ Agency
     2. วาง Price ID ที่ได้ลงในหน้า Site Content → "💳 ราคาสมาชิก Agent/เจ้าของบ้าน"
     3. ตั้งค่า secret `STRIPE_SECRET_KEY` และ `STRIPE_WEBHOOK_SECRET` ผ่าน Codespace
        (`firebase functions:secrets:set STRIPE_SECRET_KEY` แล้ว `firebase functions:secrets:set STRIPE_WEBHOOK_SECRET`)
     4. `firebase deploy --only functions` แล้วนำ URL ของ `stripeWebhook` ไปตั้งเป็น Webhook endpoint ใน Stripe
        Dashboard (เลือก event: `checkout.session.completed`, `customer.subscription.updated`,
