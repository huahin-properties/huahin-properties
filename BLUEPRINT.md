...
- **สถานะตอนนี้**: ✅ เพิ่ม UI + บันทึกลง Firestore แล้วที่ `Sell.dc.html` (private, ไม่โชว์สาธารณะ)

**ถ้าไม่ติ๊กตอนแรก — ชวนซ้ำภายหลังแบบไม่รบกวน (persistent nudge)**: เมื่อมีหน้า self-serve dashboard ให้ผู้ลง
ประกาศจัดการทรัพย์ของตัวเอง (ยังไม่ได้สร้าง — ดูหมวด 11 ชุด A ข้อ 5) ให้มีแบนเนอร์เตือนเบาๆ ค้างอยู่ในหน้าจัดการ
ทรัพย์แต่ละหลังที่ยังไม่เปิดใช้ Assisted Sale ("ยังขายไม่ได้ใช่ไหม? ให้ทีมช่วยขายเพิ่มไหม") ยิ่งทรัพย์ค้างขายนาน
ยิ่งควรโชว์เด่นขึ้น — เมื่อกดยอมรับจากแบนเนอร์นี้เมื่อไหร่ ก็เข้า workflow เดียวกับที่ติ๊กตอนแรก **ยังไม่ได้สร้าง
(รอหน้า self-serve dashboard ก่อน)**

**Workflow ติดตามหลังยอมรับ**: เมื่อผู้ลงประกาศติ๊กยอมรับ (ตอนแรกหรือภายหลังจากแบนเนอร์) ระบบควรเริ่มขั้นตอน
ติดตามอัตโนมัติ — ส่งอีเมลแจ้งเริ่มดำเนินการ + มีสถานะ step-by-step (เช่น "รับเรื่องแล้ว" → "กำลังหาลูกค้า" →
"นัดดูบ้าน" → "เจรจา" → "ปิดการขาย") ให้ Admin เห็นภาพรวมว่าทรัพย์หลังไหนอยู่ขั้นตอนไหน **ยังไม่ได้ออกแบบ
รายละเอียดสุดท้าย/ยังไม่ได้สร้าง** — ตอนนี้แค่บันทึก opt-in + % ไว้ก่อน Admin ติดตามเองนอกระบบชั่วคราวจนกว่าจะ
ออกแบบหน้า workflow tracker จริง
...
7. ~~ทาง 4 — แพ็กเกจ VIP ทั้งฝั่งเจ้าของบ้าน (เงิน/ทอง/เพชร) และฝั่ง Agent (สมัคร VIP subscription)~~ ✅
   **เสร็จแล้ว (แกนหลัก)** — ฝั่งเจ้าของบ้าน: ปุ่ม "🥈/🥇/💎 VIP" ต่อประกาศในหน้า Admin Dashboard (แอดมินกดแทน
   ...
   - **ยังไม่ได้ทำ**: กติกา Buyer Registration/Agent Code (ป้องกันข้อพิพาทระหว่าง Agent), badge "VIP" บนการ์ด
     สาธารณะ, หน้าโปรไฟล์ Agent แสดง Verified VIP, ระบบ teaser ล่อใจ (จำนวนบ้านในพูลที่ยังเข้าไม่ถึง) — ยังไม่
     สร้างเพราะต้องรอดูว่ามี Agent สมัครจริงก่อนจึงจะออกแบบกติกาป้องกันข้อพิพาทให้ตรงกับสถานการณ์จริง
