...
แพ็กเกจใดๆ ก่อน — ระบบตั้ง tier เป็น "ฟรี" ให้อัตโนมัติ สมัครเสร็จพาไปหน้า Lister Billing ทันที ที่นั่นแสดง
ตารางราคาแบบคอลัมน์เทียบกัน 4 ช่อง (ฟรี/②/③/ติดต่อเรา) ให้เห็นครบพร้อมกันในหน้าเดียว (สไตล์ pricing table
มาตรฐาน) — คอลัมน์ฟรีระบุชัดว่าเป็น "แพ็กเกจปัจจุบัน" ไม่ต้องกดอะไรก็ใช้งานได้เลย ส่วนคอลัมน์ ②③ กดอัปเกรด/
ตัดบัตรได้ทันที คอลัมน์สุดท้ายเป็นปุ่มติดต่อเราสำหรับความต้องการพิเศษ/ปริมาณมาก
...
**หมายเหตุ**: เดิมออกแบบเป็นระบบแยก (แพ็กเกจโพสต์ Basic/Pro/Agency + VIP pool Silver/Gold/Diamond ที่ stack กัน
ได้อิสระ) — ตอนนี้**รวมเป็นบันไดขั้นเดียวต่อเนื่อง 5 ระดับแล้ว** (ฟรี → Premium → VIP เงิน → VIP ทอง → VIP เพชร)
ตามตารางราคา/เกณฑ์ปลดล็อกในหมวด 2 ทาง 2 ด้านบน — ง่ายต่อความเข้าใจของ Agent กว่าเดิมมาก ไม่ต้องคิดสองระบบ
พร้อมกัน รายละเอียดกลไกด้านล่างนี้ (ฝั่งเจ้าของบ้าน, กติกา Buyer Registration ฯลฯ) ยังใช้ได้เหมือนเดิม:
...
7. ~~ทาง 4 — แพ็กเกจ VIP ทั้งฝั่งเจ้าของบ้าน (เงิน/ทอง/เพชร) และฝั่ง Agent (สมัคร VIP subscription)~~ ✅
   **เสร็จสมบูรณ์แล้ว (รวมกติกาป้องกันข้อพิพาท)** — ฝั่งเจ้าของบ้าน: ปุ่ม "🥈/🥇/💎 VIP" ต่อประกาศในหน้า Admin Dashboard (แอดมินกดแทน
   เจ้าของบ้าน เหมือน Featured boost) จ่ายครั้งเดียว/30 วัน ผ่าน `createVipCheckoutSession` (dynamic price_data)
   ตั้งราคาที่ Site Content → "💎 แพ็กเกจ VIP" — ฝั่ง Agent: หน้า Lister Billing เพิ่มส่วน "Agent VIP" ให้สมัคร
   subscription เงิน/ทอง/เพชร (ต้องสร้าง Stripe Product/Price จริงเพราะเป็นรายเดือน ใส่ Price ID ที่ Site
   Content) — ทั้งสองฝั่งเก็บ `vipTier` แยกจาก `tier` (แพ็กเกจโพสต์ปกติ) เพื่อให้ stack กันได้ตามที่ออกแบบไว้
   - **สวิตช์ Rollout Level (1-4)** ที่ Site Content — ควบคุมว่า Agent VIP section โผล่ในหน้า Lister Billing/
     ปุ่ม VIP โผล่ใน Admin Dashboard หรือยัง (ระดับ 3 = เปิดเงิน/ทอง, ระดับ 4 = เปิดครบรวมเพชร) ปลดทีละระดับ
     ได้โดยไม่ต้องแก้โค้ด ตรงกับแผน "เปิดตัวทีละสเต็ป" ที่คุยกันไว้
   - **กติกา Buyer Registration + Agent Code** ✅ เสร็จแล้ว (กรกฎาคม 2026) — Agent ที่มีระดับ VIP ได้รหัส
     Agent เฉพาะตัวอัตโนมัติ (เช่น HHP-A1234, สร้างครั้งแรกตอนโหลด Lister Dashboard) และมีปุ่ม "👤 ลงทะเบียน
     ผู้ซื้อ" ต่อทรัพย์แต่ละใบ (เฉพาะ Agent VIP) — ระบบเช็คซ้ำอัตโนมัติจากเบอร์โทร/อีเมล คุ้มครอง 60 วัน ถ้ามี
     คนอื่นลงทะเบียนไว้ก่อนจะแจ้งเตือนพร้อมวันที่+รหัส Agent คนก่อน (Firestore collection `buyerRegistrations`)
   - **Badge "VIP" บนการ์ดสาธารณะ** ✅ เสร็จแล้ว — ทรัพย์ที่มี `vipTier`+`vipUntil` ยังไม่หมดอายุ โชว่ป้าย "💎 VIP"
     บนการ์ดที่หน้า Home + Search Results (ซ้อนใต้ป้ายสถานะ/Featured)
   - **หน้าโปรไฟล์ Agent แสดง Verified VIP** ✅ เสร็จแล้ว — Agent Profile.dc.html โชว่ป้าย "🥈/🥇/💎 Verified
     VIP" ข้างชื่อ ถ้า Agent มีระดับ VIP
   - **ระบบ teaser ล่อใจ** ✅ เสร็จแล้ว — หน้า Lister Billing ส่วน "Agent VIP" (เดิมมีแค่โค้ดหลังบ้าน ไม่มี UI จริง
     — สร้าง UI ให้ครบคราวนี้) แสดงจำนวนบ้านในพูลแต่ละระดับ ถ้ายังไม่ปลดล็อกจะเห็นแค่ "🔒 มีบ้าน X หลัง —
     อัปเกรดเพื่อดูรายละเอียด" ถ้าปลดล็อกแล้วเห็นตัวเลขจริง (จำนวนบ้าน + จำนวน Agent ระดับเดียวกันขึ้นไป) ดึงจาก
     ข้อมูลจริงผ่าน `fetchVipPoolStats()` ไม่ใช่ตัวเลขหลอก
   - **ยังไม่ได้ทำ**: ฝั่งเจ้าของบ้านเห็นตัวเลข teaser คู่กัน (จำนวน Agent VIP แต่ละระดับ) ในหน้า Admin/Sell —
     ตอนนี้มีแค่ฝั่ง Agent เห็น teaser
   - **16 ก.ค. 2026 (รอบ 3) เพิ่มเติม**: หน้าเว็บสมาชิกไม่พาลูกค้าหลุดไปเว็บหลักอีกต่อไป — คลิกดูทรัพย์เปิด
     เป็นป็อปอัพในหน้าเดียวกัน เลื่อนดูรูปได้ครบทุกรูป, เพิ่มตัวเลือกภาษา 8 ภาษา (มีผลกับ UI+ป็อปอัพ+แชท AI),
     เพิ่มตัวกรอง 4 ช่อง (ประเภท/โซน/ห้องนอน/ราคา), AI ในแชทตอบสั้นลงเป็นค่าเริ่มต้นและโชว์การ์ดทรัพย์ให้คลิกดู
     เองแทนพิมพ์รายละเอียดยาว, คำอธิบายทรัพย์ที่สมาชิกโพสต์เองแปล 8 ภาษาอัตโนมัติทันทีตอนกด "บันทึก" (เหมือน
